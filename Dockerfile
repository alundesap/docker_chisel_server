...
# prepare go env
ENV GOPATH /go
ENV NAME docker_chisel_server
ENV PACKAGE github.com/alundesap/$NAME
...
RUN set -ex \
	...
	&& cd $PACKAGE_DIR \
	&& go build \
		-ldflags "-X github.com/alundesap/docker_chisel_server/share.BuildVersion=$(git describe --abbrev=0 --tags)" \
